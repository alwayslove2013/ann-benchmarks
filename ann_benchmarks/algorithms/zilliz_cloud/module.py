...
class ZillizCloud(BaseANN):
    def __init__(self, metric, dim, db_config):
        self.dim = dim
        _metric = METRIC_MAPPING.get(metric, None)
        print(f"metric: {metric} => {_metric}")
    ...
    def batch_query(self, X: np.array, n: int):
        print(f"batch query - concurrencies: {self.concurrencies}")
